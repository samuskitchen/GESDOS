...
import com.agileillutions.gesdos.dataaccess.dao.ICargoDAO;
...
public class CargoLogic implements ICargoLogic {
    ...
    public void saveCargo(Cargo entity) throws Exception {
        try {
            if (entity.getCarCod() == null) {
                throw new ZMessManager().new EmptyFieldException("carCod");
            }

            if ((entity.getCarCod() != null) &&
                    (Utilities.checkWordAndCheckWithlength(entity.getCarCod(), 4) == false)) {
                throw new ZMessManager().new NotValidFormatException("carCod");
            }

            if (entity.getCarDes() == null) {
                throw new ZMessManager().new EmptyFieldException("carDes");
            }

            if ((entity.getCarDes() != null) &&
                    (Utilities.checkWordAndCheckWithlength(entity.getCarDes(),
                        30) == false)) {
                throw new ZMessManager().new NotValidFormatException("carDes");
            }

            if (getCargo(entity.getCarCod()) != null) {
    ...
    public List<CargoDTO> getDataCargo() throws Exception {
        try {
            ...
            for (Cargo cargoTmp : cargo) {
                ...
                cargoDTO2.setCarCod(cargoTmp.getCarCod());
                cargoDTO2.setCarDes((cargoTmp.getCarDes() != null)
                    ? cargoTmp.getCarDes() : null);
    ...
    public Cargo getCargo(String carCod) throws Exception {
        ...
        try {
            entity = cargoDAO.findById(carCod);
        } catch (Exception e) {
